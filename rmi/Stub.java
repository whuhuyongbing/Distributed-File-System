...
public abstract class Stub
{
    ...
    public static <T> T create(Class<T> c, Skeleton<T> skeleton) throws UnknownHostException
    {
        ...
        if (skeleton.getSocketAddr() == null && skeleton.getThread() == null) {
            throw new IllegalStateException("Unable to create stub");
        }
        /** see if remote interface*/
        Utils.remoteInterface(c);
        InetSocketAddress sockAddr = new InetSocketAddress(
                skeleton.getSocketAddr().getHostName(), skeleton.getSocketAddr().getPort());
    ...
    public static <T> T create(Class<T> c, Skeleton<T> skeleton, String hostname)
    {
        ...
        if (skeleton.getSocketAddr() == null) {
            throw new IllegalArgumentException("the skeleton has not been assigned a port");
        }
        /** see if remote interface*/
        Utils.remoteInterace(c);
        InetSocketAddress sockAddr = new InetSocketAddress(hostname, skeleton.getSocketAddr().getPort());
        ProxyHandler<T> proxyHandler = new ProxyHandler<>(c, sockAddr);
